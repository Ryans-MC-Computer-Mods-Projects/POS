...
local settings = {
	theme = {
    	desktopTextColour = 1,
    	taskbarBGColour = 1,
    	defDeselTextColour = 256,
    	taskbarShownWinIconColour = 8,
    	defBackgroundColour = 1,
    	defSelBGColour = 256,
    	taskbarTextColour = 32768,
    	defTextColour = 32768,
    	backgroundColour = 512,
    	selWindowTextColour = 1,
    	windowTextColour = 128,
    	windowColour = 256,
    	selBackgroundColour = 8,
    	defSelTextColour = 32768,
    	selTitleBarColour = 128,
	},
	time = {
		autoDST = true,
		autoTime = true,
		...
		openProgramAsSP = function(IDtC, name, X, Y, Sx, Sy, HasWinDecor, ...)
			if processes[IDtC] then
				if processes[IDtC].SP then
					...
					X = X or math.ceil(six / 2) - math.floor(Sx / 2)
					Y = Y or math.ceil(siy / 2) - math.floor(Sy / 2)
					return true, addProcress(name, false, X, Y, Sx, Sy, true, true, table.unpack({...}))
				end
			end
			return false
	    end,
...
local Tx, Ty = OGTerm.getSize()
taskbarTask = addProcress("temp/Taskbar.lua", false, 1, 1, 3, Ty, false, true)
desktopTask = addProcress("temp/Desktop.lua", false, 4, 1, Tx - 3, Ty, false, true)
addProcress("temp/System.lua", true)
term.setBackgroundColour(settings.theme.backgroundColour)
term.clear()
buffer.reposition(1, 1, Tx, Ty)
